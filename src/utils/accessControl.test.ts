...
describe('assessment interpretation audit access', () => {
  it('exposes the audit drawer to qs admins and platform admins', () => {
    expect(buildAccessContext(['qs:admin']).capabilities.has('audit_interpretation')).toBe(true)
    expect(buildAccessContext(['platform_admin']).capabilities.has('audit_interpretation')).toBe(true)
  })

  it('does not expose the audit drawer to process-only operators', () => {
    expect(buildAccessContext(['qs:assessment_operator']).capabilities.has('audit_interpretation')).toBe(false)
  })
})

describe('norm-table access', () => {
  it('allows content managers to read and import norm tables', () => {
    const access = buildAccessContext(['qs:content_manager'])
  ...
  it('does not expose norm-table administration to result reviewers', () => {
    const access = buildAccessContext(['qs:result_reviewer'])
...
describe('independent business roles', () => {
  it('combines independent jobs without granting audit capability', () => {
    const access = buildAccessContext(['qs:assessment_operator', 'qs:result_reviewer'])
  ...
  it('separates process operations from professional results', () => {
    const ops = buildAccessContext(['qs:assessment_operator'])
    ...
    const reviewer = buildAccessContext(['qs:result_reviewer'])
  ...
  it('requires result capability even when the user is a clinician', () => {
    ...
    const clinicianOperator = buildAccessContext(['qs:assessment_operator'])
    expect(routeAllowsAccess(resultRoute, clinicianOperator, true)).toBe(false)

    const clinicianReviewer = buildAccessContext(['qs:result_reviewer'])
