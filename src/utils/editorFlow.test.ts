...
describe('personality editor flow', () => {
  it('maps personality routes to step keys', () => {
    expect(getPersonalityStepFromPath('/typology/info/m1')).toBe('create')
    expect(getPersonalityStepFromPath('/typology/create/m1/0')).toBe('edit-questions')
    expect(getPersonalityStepFromPath('/typology/routing/m1')).toBe('set-routing')
    expect(getPersonalityStepFromPath('/typology/definition/m1')).toBe('edit-definition')
    expect(getPersonalityStepFromPath('/typology/publish/m1')).toBe('publish')
  ...
  it('maps step keys to stable URLs', () => {
    ...
    expect(personalityEditorFlowConfig.getPathForStep('publish', 'm1')).toBe('/typology/publish/m1')
    expect(personalityEditorFlowConfig.listPath).toBe('/typology')
  })
