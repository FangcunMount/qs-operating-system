...
import { Card, Row, Col, Button, Typography, Space, Collapse } from 'antd'
import {
  ExperimentOutlined,
  RightOutlined,
  DownOutlined,
          ...
          <div className="home-header">
            <div className="header-content">
              <div className="home-welcome-copy">
                <span className="home-eyebrow">QLUME · 运营工作台</span>
                <Title level={2} className="header-title">掌握进展，从这里开始。</Title>
                <Text className="header-subtitle">连接门店服务与测评运营，让每一份进展清晰可见。</Text>
                <Button type="primary" icon={headerAction.icon} onClick={() => history.push(headerAction.path)} className="header-action">
                  {headerAction.text}
                </Button>
              </div>
              <div className="home-hero-art" aria-hidden="true"><span /><span /><ShopOutlined /></div>
            </div>
          </div>
          <div className="home-container"><OperationsPanel compact /></div>
        </section>
        <aside className="home-workspace__side">
          <Card title={<Space><FolderOutlined /><span>功能入口</span></Space>} className="home-actions-card">
            <Text type="secondary">常用管理，一步直达</Text>
            <div className="home-action-grid">
              {orderedQuickLinks.map(link => <button className="home-action-tile" onClick={() => history.push(link.path)} key={link.path}>
                <span className="home-action-tile__icon" style={{ color: link.color }}>{link.icon}</span>
                <strong>{link.title}</strong>
                <span>{link.description}</span>
                <RightOutlined className="home-action-tile__arrow" />
              </button>)}
            </div>
          </Card>
          <Card className="home-service-note" title="读懂你的运营数据">
            <div><i /><strong>看当前服务</strong><p>当前服务人数反映受试者现在归属的门店。</p></div>
            <div><i /><strong>看历史开展</strong><p>提交和完成量记录测评开展时的门店，转店后保留历史。</p></div>
            <div><i /><strong>按权限处理</strong><p>业务入口和操作以当前账号获得的权限与门店范围为准。</p></div>
          </Card>
        </aside>
      </div>
      {userStore.accessContext.capabilities.has('manage_content') && <Collapse ghost className="home-guidance">
        <Collapse.Panel header="量表配置指南" key="content-guide">
          {/* 使用指南 */}
          <Row gutter={[16, 16]}>
            <Col xs={24}>
              <Card
