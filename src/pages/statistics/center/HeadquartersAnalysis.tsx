...
const StatisticsCenterPage: React.FC = () => {
  ...
  const fetchData = () => setRevision(value => value + 1)
  useEffect(() => {
    let cancelled = false
    setLoading(true); setError(''); setOverview(null)
    getOverviewStatistics(buildParams()).then(([err, response]) => {
      if (err || !response?.data) throw err || new Error('获取统计概览失败')
      if (!cancelled) setOverview(response.data)
    }).catch(err => {
      if (!cancelled) setError(extractErrorMessage(err, '获取总部分析失败，请稍后重试'))
  ...
  useEffect(() => {
    let cancelled = false
    if (activeTab !== 'clinicians') return
    const params = { ...buildParams(), page: 1, page_size: 10 }
    setClinicianLoading(true); setEntryLoading(true); setClinicianError(''); setEntryError('')
    setClinicians([]); setEntries([])
    listClinicianStatistics(params).then(([err, response]) => {
      if (err || !response?.data) throw err || new Error('医生统计不可用')
      if (!cancelled) setClinicians(response.data.items || [])
    }).catch(err => {
      if (!cancelled) setClinicianError(extractErrorMessage(err, '医生统计加载失败'))
    }).finally(() => { if (!cancelled) setClinicianLoading(false) })
    listAssessmentEntryStatistics(params).then(([err, response]) => {
      if (err || !response?.data) throw err || new Error('入口统计不可用')
      if (!cancelled) setEntries(response.data.items || [])
    }).catch(err => {
      if (!cancelled) setEntryError(extractErrorMessage(err, '入口统计加载失败'))
    }).finally(() => { if (!cancelled) setEntryLoading(false) })
    return () => { cancelled = true }
  }, [activeTab, buildParams, revision])
    ...
    <div>
      <Space direction="vertical" size={16} style={{ width: '100%' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap' }}>
          <div>
            <Title level={4} style={{ marginBottom: 4 }}>总部专题</Title>
            <Text type="secondary">保留接入、医生与计划专题；沿用各自统计口径和权限，此处不按运营分析的门店筛选联动。</Text>
          </div>
          <Space wrap>
            <Select
              value={preset}
        ...
        <Tabs activeKey={activeTab} onChange={setActiveTab}>
          <TabPane tab="统计中心概览" key="overview">
            {loading ? <Card loading /> : error ? <Alert type="error" showIcon message="总部分析暂不可用"
              description={error} action={<Button onClick={fetchData}>重试</Button>} /> : <>
          ...
          <TabPane tab="临床人员维度" key="clinicians">
            <Space direction="vertical" size={16} style={{ width: '100%' }}>
              {clinicianLoading ? <Card loading /> : clinicianError ? <Alert type="warning" showIcon message="医生统计暂不可用"
                description={clinicianError} action={<Button onClick={fetchData}>重试</Button>} /> : <>
                <Card className="dashboard-summary" loading={false}>
                  <div className="dashboard-summary__header">
                    <div>
                  ...
                  <div className="dashboard-kpi-grid dashboard-kpi-grid--four">
                    <div className="dashboard-kpi">
                      ...
                      <div className="dashboard-kpi__value">{overview ? formatNumber(overview.dimension_analysis.clinician_count) : '—'}</div>
              ...
              {entryLoading ? <Card loading /> : entryError ? <Alert type="warning" showIcon message="入口统计暂不可用"
                description={entryError} action={<Button onClick={fetchData}>重试</Button>} /> : <>
                <Card className="entry-supplement" loading={false}>
                  <div className="entry-supplement__header">
                    ...
                    <Space size={16} wrap>
                      <Statistic title="机构入口" value={overview?.dimension_analysis.entry_count ?? '—'} />
