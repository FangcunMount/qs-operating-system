...
export const DeliveryResolutionDrawer: React.FC<DeliveryResolutionDrawerProps> = ({
  ...
  const submit = async () => {
    if (!canResolve(review, target) || !review || !target || !target.event_id || !target.delivery_attempts || receipt) return
    const values = await form.validateFields()
    const id = String(values.request_id).trim()
    const command: DeliveryResolutionRequest = {
      request_id: id,
    ...
    setChecking(false)
    if (!committed) {
      setError(`${extractErrorMessage(requestError, '结案结果尚未确认')}。请保留本次结案编号，先查询回执；再次提交须沿用相同编号和原输入。`)
    }
  }

  const recheck = async () => {
    const id = String(form.getFieldValue('request_id') || requestID).trim()
    if (!id) return
    setChecking(true)
    const committed = await checkReceipt(id)
    setChecking(false)
  ...
  return (
    <Drawer title="按业务事实核实传输死信" visible={visible} width={560} onClose={onClose} forceRender
      footer={<Space>
        ...
        <Button type="primary" loading={submitting} disabled={!canResolve(review, target) || Boolean(receipt)} onClick={() => void submit()}>
          核实后结案
        </Button>
      </Space>}
    >
      <Space direction="vertical" size={16} style={{ width: '100%' }}>
        ...
        <Form form={form} layout="vertical">
          <Form.Item name="request_id" label="本次结案编号"
            ...
            <Input onChange={(event) => setRequestID(event.target.value)} />
          </Form.Item>
          <Form.Item name="reason" label="业务核实说明"
            rules={[{ required: true, whitespace: true, message: '请填写核实说明' }]}>
            <Input.TextArea rows={3} placeholder="说明已核对的报告与关注业务事实" />
          </Form.Item>
