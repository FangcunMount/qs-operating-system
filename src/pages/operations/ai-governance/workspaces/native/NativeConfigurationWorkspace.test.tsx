...
jest.mock('../product/SolutionHome', () => ({ SolutionHome: ({ onEdit }: any) => <button onClick={() => onEdit({
...
jest.mock('./PromptDraftWorkspace', () => ({ PromptDraftWorkspace: ({ onContinue }: any) =>
  <button onClick={() => onContinue({ identity: 'prompt', version: 'v7' })}>冻结并继续</button> }))
jest.mock('../product/PrepareSolution', () => ({ PrepareSolution: ({ selection, semantic, onReady }: any) =>
  <button onClick={() => onReady({ suite: { id: 'suite', version: 'v7', fingerprint: 'suite-hash' },
    manifest: { generation_route: selection.route } })}>
    准备 {selection.prompt?.version} {semantic?.version}
  </button> }))
jest.mock('./NativeEvaluationWorkspace', () => ({ NativeEvaluationWorkspace: ({ selection }: any) =>
  <div>测试配置 {selection.suite?.version} {selection.generation_route?.version} {selection.semantic_route?.version}</div> }))
