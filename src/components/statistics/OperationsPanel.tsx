...
const OperationsPanel: React.FC<{ compact?: boolean }> = observer(({ compact = false }) => {
  ...
  const identity = JSON.stringify([userStore.currentUser || null, userStore.accessContext])
  ...
  const report = <>
    {loading && view !== 'guide' && <Card className="operations-surface" aria-label="正在加载运营统计">
    ...
    {!compact && view === 'guide' ? <OperationsGuide data={data} /> : data && <>
      <div className="operations-context">
        ...
        <span>开展统计：{data.from} 至 {data.workload_through}</span>
      ...
      <div className={`operations-chart-grid ${view === 'stores' ? 'operations-chart-grid--wide' : ''}`}>
        ...
        {!compact && view === 'overview' && <Card className="operations-surface operations-reading" title="这份报表怎么看">
          <div className="operations-reading__item">
            <span className="operations-reading__dot" />
            <div>
              <strong>先看服务规模</strong>
              <p>当前服务人数随转店更新，反映现在服务的受试者。</p>
            </div>
          </div>
          <div className="operations-reading__item">
            <span className="operations-reading__dot operations-reading__dot--green" />
            <div>
              <strong>再看期间开展量</strong>
              <p>提交和首次成功分别计数，归入开始作答时的门店。</p>
            </div>
          </div>
          <div className="operations-reading__item">
            <span className="operations-reading__dot operations-reading__dot--violet" />
            <div>
              <strong>按同一范围比较</strong>
              <p>切换门店分析时保留当前日期和门店选择，不增加明细访问权限。</p>
            </div>
          </div>
    ...
    {!compact && <>
      <div className="operations-filters">
        <div className="operations-filter-group">
          <span className="operations-filter-label">统计期间</span>
      ...
      <Tabs className="operations-navigation" destroyInactiveTabPane activeKey={view} onChange={key => setView(key as View)}>
        ...
        <Tabs.TabPane tab="门店分析" key="stores">{view === 'stores' && <StoreAnalysis identity={identity} revision={revision}
          query={{ ...(range ? { from: range[0], to: range[1] } : {}),
            ...(selected.length ? { store_ids: selected.join(',') } : {}) }} history={report} />}</Tabs.TabPane>
