...
const riskRuleOutcome = (rule: { OutcomeCode?: string; Level?: string; Summary?: string; Description?: string }) => {
  const code = asString(rule.OutcomeCode || rule.Level)
  return {
    ...riskLevelOutcome(code),
    Summary: asString(rule.Summary),
    Description: asString(rule.Description),
  }
}

/**
 * DefinitionV2 keeps a model-wide outcome registry. The JSON editor accepts
 * canonical score-range rules directly, so make sure their referenced codes
 * are registered before PUT. Existing outcome content is intentionally kept:
 * the registry may also be shared by non-scale conclusion kinds.
 */
export const ensureScaleDefinitionOutcomeRegistry = (definition: RawDefinition): RawDefinition => {
  ...
  conclusionsOf(definition).forEach((conclusion: any) => {
    asArray(readField(conclusion, 'Rules', 'rules')).forEach((rule: any) => {
      const code = asString(readField(rule, 'OutcomeCode', 'outcome_code'))
        || asString(readField(rule, 'Level', 'level'))
      if (code) referencedCodes.add(code)
    })
  })

  setCaseField(next, 'Outcomes', 'outcomes', [
    ...existingOutcomes,
    ...Array.from(referencedCodes).filter((code) => !existingCodes.has(code)).map(riskLevelOutcome),
  ])
  return next
}
...
const riskConclusion = (factor: any) => {
  ...
  const outcomesByCode = new Map<string, ReturnType<typeof riskRuleOutcome>>()
  rules.forEach((rule) => {
    const code = asString(rule.OutcomeCode || rule.Level)
    if (code && !outcomesByCode.has(code)) outcomesByCode.set(code, riskRuleOutcome(rule))
  })
  return {
    Kind: 'risk',
    FactorCode: factor.code,
    Rules: rules,
    Outcomes: Array.from(outcomesByCode.values()),
  }
}
...
export const replaceScaleDefinitionFactors = (definition: RawDefinition, factors: any[]): RawDefinition => {
  ...
  return ensureScaleDefinitionOutcomeRegistry(next)
}
